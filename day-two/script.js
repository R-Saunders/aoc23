...
function parseGameData(inputString) {
	...
	// Split first array item
	const thirdSplit = secondSplit[0].split(" ");

	function parseRounds(inputArray) {
		...
		inputArray.forEach((round) => {
			...
			// Split the string into an array of cube and quantity pairs
			const cubesDrawn = round.split(", ");

			// Create an object to store the result for the current string
			const roundObject = {};

			// Loop through each cube pair and add it to the object
			cubesDrawn.forEach((pair) => {
	...
	const roundsObject = parseRounds([secondSplit[1], ...firstSplit]);

	// Create a game object
	const gameObject = { [thirdSplit[0]]: parseInt(thirdSplit[1]) };

	// Put back into arrays
	const game = [gameObject, roundsObject];

	return game;
}

// loop over array
function parseAllGamesData(inputArray) {
	for (i = 0; i < inputArray.length; i++) {
		let itemParsed = parseGameData(inputArray[i]);
