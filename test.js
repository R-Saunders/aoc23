...
function parseGameData(inputString) {
  ...
  rounds.forEach((round, index) => {
    // Extract game number and round data
    const [gameNumber, roundData] = round.trim().split(':');
    
    // Extract color counts for each round
    const colorCounts = roundData.trim().split(',').map(count => count.trim());
...
// Example usage
const inputString = "Game 1: 4 blue, 16 green, 2 red; 5 red, 11 blue, 16 green; 9 green, 11 blue; 10 blue, 6 green, 4 red";
const gamesData = parseGameData(inputString);
